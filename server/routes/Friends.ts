...
interface Relationship extends Model {
  isConfirmed: boolean,
...
Friends.get('/api/getFriends/:id', async (req: Request, res: Response) => {
  ...
  if (allFriendships) {
    ...
    const allFriendsIds = allFriendships.map((friendship: Relationship) => {
...
Friends.post('/api/requestFriend', async (req: Request, res: Response) => {
  ...
  try {
    const newRelationship = await Join_friend.create({ requester_userId, recipient_userId, isConfirmed: null })
