...
ImageRouter.post('/upload', async (req: Request, res: Response) => {
  ...
  try {
    console.log('b4 MW, req', req, '!REQ BODY!', req.body)
    await runMiddleware(req, res, myUploadMiddleware);
    // file object added to req after going thru multer
    console.log('after MW, req.file', req.file)
...
ImageRouter.put('/save/:ownerId', async (req: Request, res: Response) => {
  const { latitude, longitude, isThrow, isPin, isCostume, description } =
    req.body.options;
  const { ownerId } = req.params;

  try {
    await Photo.update(
      { latitude, longitude, isThrow, isPin, isCostume, ownerId, description },
      { where: { photoURL } }
    );
    const matchedLatLngPhotos = await Photo.findAll({
      where: { latitude, longitude },
    });
    const matchedLatLngPin = await Pin.findOne({
      where: { latitude, longitude },
    });

    await Promise.all(
      matchedLatLngPhotos.map(async (photo) => {
        try {
