...
const CreateFriend: React.FC<CreateFriendProps> = ({}) => {
  ...
  // TODO: Add feedback for creating friend
  ...
  const handleSubmit = async () => {
    try{
      const friendRequestResponse = await axios.post('/api/friends/createFriendRequest', {
        requesterId: user.id,
        email
      })
  ...
  return (
    <div >
      <Form className='d-flex flex-column justify-content-center'>
        ...
        {alert.isDisplayed && <Alert variant={alert.variant}>
          {alert.text}
        </Alert>}
      </Form>
    </div>
